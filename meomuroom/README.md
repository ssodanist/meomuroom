...
### 배포 환경(서버리스)에서도 로그인이 유지되는 이유

Netlify 같은 서버리스 환경은 요청마다 완전히 다른 서버 인스턴스가 뜰 수 있어서,
`lib/mockData.ts`의 메모리 배열(회원 목록 등)이 요청 사이에 초기화될 수 있습니다.
그래서 로그인 세션 자체는 메모리를 전혀 믿지 않고, `mm_session` / `mm_link_<provider>`
쿠키 안에 회원 프로필 전체를 그대로 담아둡니다 — 어떤 서버 인스턴스가 요청을 처리하든
쿠키만으로 로그인 상태와 내 정보(별명·지역·관심사 등)를 완전히 재구성할 수 있습니다
(회원가입/로그인/로그아웃/탈퇴·복구, 관심사 등록/해제, 안부 설정까지 모두 이 방식).

다만 **다른 회원이 쓴 글·댓글·모임·토론·Q&A**처럼 여러 사람이 함께 보는 데이터는
성격상 쿠키에 담을 수 없어 여전히 서버 메모리에만 저장됩니다 — 즉, 방금 쓴 글이 서버
인스턴스가 바뀌는 시점에 다른 방문자에게 안 보일 수 있습니다. 이 부분까지 완전히
안정적으로 만들려면 아래 "Supabase 연결하기"를 진행해 실제 데이터베이스에 저장해야
합니다 (로그인 상태 자체는 이미 안정적입니다).
